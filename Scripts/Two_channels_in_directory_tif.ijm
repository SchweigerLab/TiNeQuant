...
for (i = 0; i < fileList.length; i++)
{
	...
	//Subtract channel 1 from the channel 2 to get rid of most of the autofluorescence (Erythrozytes,...)
	...
	//The particle analyzer gives out an inverted LUT, the dilate/erode command does react LUT dependent not greyvalue dependent
	run("Invert LUT");
	//Dilate cycles followed by erode cylcles connect non connected signal
	...
	saveAs("Tiff", ""+outputDir+"/Channel_1_stacks/"+filename+"");
	...
	saveAs("Tiff", ""+outputDir+"/Channel_2_stacks/"+filename+"");
	rename("Neurons_"+a+"");
	//Duplicate the output for lobe volume quantification
	run("Duplicate...", "duplicate");
	...
	//Channel 1 quantification
	Table.set ("Nr", i*3, a);
	Table.set ("Image", i*3, "Channel_1_"+filename+"");
	Table.set ("Total length",i*3, b);
	Table.set ("Unit", i*3, "µm");
	//Channel 2 quantification 
	Table.set ("Nr", i*3+1, fileList.length+a);
	Table.set ("Image", i*3+1, "Channel_2_"+filename+"");
	Table.set ("Total length", i*3+1, c);
	Table.set ("Unit", i*3+1, "µm");
	//Lobevolume quantification
	Table.set ("Nr", i*3+2, fileList.length*2+a);
	Table.set ("Image", i*3+2, "Lobevolume_"+filename+"");
